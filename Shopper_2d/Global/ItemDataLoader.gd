...
func print_every_item():
	...
	for id in shop_items:
		print("Item: ", shop_items[id]["name"], ", type: ", shop_items[id]["type"], ", store_area: ", shop_items[id]["store_area"],
				 ", value: ", shop_items[id]["value"], ", Average Value: ", shop_items[id]["average_value"], ", unlocked: ", shop_items[id]["unlocked"])
...
func import_shop_item_data():
	...
	while not file_content.eof_reached():
		...
		if line != "":
			...
			var value = item_data[4]
			var average_value = item_data[5]
			var unlocked = item_data[6]
	...
	# Close the file
	file_content.close()
	print("Shop items loaded from tab-delimited file!")
