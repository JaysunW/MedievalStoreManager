...
func _process(_delta):
	...
	var mouse_grid_pos = tile_mouse_pos * 32 + Vector2i(16, 16)
	var tile_data : TileData = build_map.get_cell_tile_data(0, tile_mouse_pos)
		...
		if in_build_area:
			if Input.is_action_pressed("left_mouse"):
				if mouse_grid_pos not in object_dict.keys() and Gold.pay(current_build_object.data["value"]):
					...
					build_map.set_cell(0, tile_mouse_pos, 0, Vector2i(1,0))
...
func create_placeable_object():
	...
	var mouse_pos = get_global_mouse_position()
	var tile_mouse_pos : Vector2i = build_map.local_to_map(mouse_pos)
	current_build_object.position = tile_mouse_pos * 32 + Vector2i(16, 16)
	tile_map.add_child(current_build_object)
	current_build_object.data = current_build_data
	current_build_object.tile_map_reference = tile_map
