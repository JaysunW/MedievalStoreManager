...
func set_tile_signal(sig):
	sig.connect(_place_drop_at)
	
func get_object_for_type(type):
	match type:
		Enums.DropType.TILE:
			return tile_drop_scene.instantiate()
		Enums.DropType.CORAL:
			return coral_drop_scene.instantiate()
		Enums.DropType.SHELL:
			return shell_drop_scene.instantiate()
	
func _place_drop_at(pos, type):
	var drop = tile_drop_scene.instantiate()
	add_child(drop)
	print(type)
	drop.position = pos * 32
	drop.set_drop_type(type)
	drop.update_sprite()
